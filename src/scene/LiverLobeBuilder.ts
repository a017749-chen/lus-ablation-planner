...
export function containsSphereWithinIllustrativeLobe(
  ...
  if (!metadata || radius < 0) return false;
  ...
  if (distanceFromInterface < 0 || distanceFromInterface > metadata.extentX) return false;
  ...
  const conservativeRadius = radius / Math.min(
    metadata.extentX,
    metadata.radiusY,
    metadata.radiusZ
  );

  return normalizedCenterDistance + conservativeRadius < 1;
}
