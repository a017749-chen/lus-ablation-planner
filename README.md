...
- **側視線陣探頭**（`src/config/probe.ts`，幾何類似 BK 8666-RF）：陣列在尖端側面，影像面含探頭長軸，聲束垂直於陣列面，影像為矩形（寬＝陣列長 30 mm）。桿身以套管為支點轉動（俯仰、偏擺、自轉、插入長度），尖端四向彎曲。影像面、B-mode 與共平面判定都用同一個姿態計算。
...
- **徒手平面內進針**：點一個皮膚點，檢查針長、距血管、皮膚傾斜角，以及是否存在一個聲窗讓影像面同時包含這條針道；可行時探頭會轉到那個姿態。
- **皮膚可行區**：在皮膚上以顏色標示探頭孔與徒手進針點的可行區（綠／藍／青；肋骨區較暗）。

**所有探頭與導引孔數字都是未校正的預設值**（畫面會標示）。特別是導引角 30° 要量自哪個軸：量自探頭長軸時，導引線在 30 mm 線陣影像內只到約 17 mm 深；量自聲束時約 52 mm。兩者都可在介面切換，請依實際探頭確認。
...
- **B-Mode 1.5mm 薄層投影**：病灶輪廓嚴格依據 1.5 mm 聲束薄層幾何截面縮放；超出線陣矩形影像（寬 30 mm、深 0–80 mm）的部分自動裁切。
